  ...
  return (
    <section id="webdev" className="relative min-h-screen flex flex-col justify-center px-6 md:px-[10vw] py-32 overflow-hidden">
      ...
      <div className="relative z-10">
        ...
        {/* Services — each card sized to its own content, no forced stretch */}
        <div className="mt-20 grid grid-cols-1 md:grid-cols-3 gap-5 items-start">
          {services.map((service, i) => (
            <ScrollReveal key={service.num} delay={600 + i * 120}>
              <div
                className="p-8 md:p-10 rounded-[24px] border transition-all duration-500 flex flex-col group"
                onMouseEnter={() => setHoveredService(i)}
                onMouseLeave={() => setHoveredService(null)}
                data-interactive
                style={{
                  background: hoveredService === i ? 'rgba(255,255,255,0.09)' : 'rgba(255,255,255,0.045)',
                  transform: hoveredService === i ? 'translateY(-6px)' : 'translateY(0)',
                  borderColor: hoveredService === i ? 'rgba(188, 0, 0, 0.5)' : 'rgba(255,255,255,0.16)',
                  boxShadow: hoveredService === i
                    ? '0 24px 44px -14px rgba(188, 0, 0, 0.3)'
                    : '0 10px 24px -10px rgba(0, 0, 0, 0.4)',
                }}
              >
                <span
                  className="text-[10px] tracking-[3px] opacity-70 mb-5 block"
                ...
                <p className="text-[13px] opacity-55 leading-[1.7]">{service.desc}</p>
              </div>
            </ScrollReveal>
          ))}
        </div>
